...
for i in range(150): #NEW!
    railing2.append(0) #NEW!
...
for i in range( numArcs ):
    ...
    rainbowDrawings.append([]) #NEW!

    for n in range(200): #NEW!
        rainbowDrawings[i].append(0) #NEW!

    

lakeBackground = screen.create_polygon(-50, 600, 1250, 700, 1250, 710, 1250, 900, -50, 950, -50, 700, fill = "#07609a", smooth = True)
...
for f in range(1000000000):
    ...
    for i in range(numPeople):
        ...
        if dieRoll <= 4 or f > 10:
            x3Speed[i] = 0

            for f in range(11):
                if f%10 == 0:
                    x3Speed[i] = randint(-5, 5)
    ...
    for r in range(150):
        ...
        yRail = 130
        railSize = 3
        railing2[r] = screen.create_line(xRail, yRail, xRail + railSize, yRail + 15, fill = "black") #NEW!
    ...
    for n in range(200): #NEW!
        for i in range(numArcs): #NEW!
            screen.delete( rainbowDrawings[i][n] ) #NEW!
